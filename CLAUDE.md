...
| Fix formato description ticket (HTML invece di Markdown) in wm-plan | — (nessun ticket) | `plugins/wm-skills/skills/wm-plan/SKILL.md` | Il campo `description` di un ticket Orchestrator è renderizzato da un editor WYSIWYG (nessun parsing Markdown); `ticket: caso-b` e tag-mode ora costruiscono la `description` in HTML (`<h2>`/`<h3>`, `<p>`, `<ul><li>`) invece di Markdown grezzo prima del POST |
